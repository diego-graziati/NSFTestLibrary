...
void Test::execute_category_with_single_unit(mytest_report_type_t& report)
{
    ...
    status = mytest_execute_category_test(test_category, 0);

    if (status == MYTEST_TEST_CATEGORY_NOT_INITIALIZED)
    {
    ...
    if (status == MYTEST_TEST_CATEGORY_REPORT_ARRAY_NOT_INITIALIZED)
    {
    ...
    if (status == MYTEST_TEST_INDEX_BIGGER_THAN_CATEGORY_SIZE)
    {
    ...
    if (status == MYTEST_REPORT_MESSAGE_IS_TOO_LONG)
    {
...
void Test::execute_category_with_multiple_units(mytest_report_type_t& report)
{
    mytest_test_category_t test_category = nullptr;
    std::string report_message;
    uint32_t status = mytest_init_test_category(&test_category, "Test category");
    ...
    {
        status = mytest_execute_category_test(test_category, i);
...
void Test::execute_multiple_categories_with_single_unit(mytest_report_type_t& report)
{
    ...
    for (uint32_t i = 0U; i < test::MAX_TEST_CATEGORIES_ARRAY_LENGTH; i++)
    {
        status = mytest_execute_category_test(test_category[i], 0);
...
void Test::execute_multiple_categories_with_multiple_units(mytest_report_type_t& report)
{
    mytest_test_category_t test_category[test::MAX_TEST_CATEGORIES_ARRAY_LENGTH] = {0};
    std::string report_message;
    uint32_t status = MYTEST_SUCCESS;
    ...
    for (uint32_t i = 0U; i < test::MAX_TEST_CATEGORIES_ARRAY_LENGTH; i++)
    {
        for (uint32_t j = 0U; j < test::MAX_TEST_UNITS_ARRAY_LENGTH; j++)
        {
            status = mytest_execute_category_test(test_category[i], j);
